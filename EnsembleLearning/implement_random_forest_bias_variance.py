...
from random_forest_bagging_algorithm import BaggingAlgorithm
...
def two_e(training_data, testing_data):
    ...
    for i in range(total_bagged_predictors):
        rf = BaggingAlgorithm(num_iterations=num_trees, sample_size=sample_size) 
    ...
    print(f"Single tree learner = bias: {single_tree_bias}, variance: {single_tree_variance} and squared error: {single_tree_squared_error}")
    print(f"Whole forest learner = bias: {bagged_bias}, variance : {bagged_variance} and squared error: {bagged_squared_error}")
...
def main():
    ...
    two_e(training_data, testing_data)
